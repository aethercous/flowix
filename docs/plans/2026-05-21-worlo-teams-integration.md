...
**Architecture:** Reuse existing `access_codes` + `teams-auth` + `generate-teams-code` pipeline (not legacy `team_access_codes`). Add `manage-teams-code` for revoke/agent kill-switch. Electron desktop wraps `teams-app/` with the same Worlo design tokens.
...
| Piece | Status |
|-------|--------|
| `access_codes` + `code_access_logs` | Live, hashed `WORLO-*` codes |
...
6. **`worlo-teams-desktop/`:** Electron app + build scripts for Windows/macOS.
...
- [ ] `cd worlo-teams-desktop && npm install && npm run build:win` / `build:mac`
