...
**Production homepage:** https://worlo.site
...
4. Add **Authorized redirect URIs** — add **both**:
   ...
   - `https://worlo.site/auth/google-callback.html` (**primary** — shows Flowix on Google's account chooser)
...
6. **OAuth consent screen → Branding**: set App name to **Flowix** (or worlo), upload logo, and add:
   - Home page: `https://worlo.site`
   - Privacy policy: `https://worlo.site/privacy-policy.html`
   - Terms: `https://worlo.site/terms-of-service.html`
7. Under **Authorized domains**, add and verify `worlo.site` in [Google Search Console](https://search.google.com/search-console).
...
1. Open https://worlo.site → **Sign in** → **Continue with Google**.
...
- **Branded Google OAuth** uses `https://worlo.site/auth/google-callback.html` so users see **Flowix** / **worlo.site** instead of `utofnywijqsozjqmkhcn.supabase.co` on Google's account chooser.
