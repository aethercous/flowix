...
-- -- 4b. User agents (definitions persisted per account) --------------------
...
create policy "agents_owner_select" on public.agents for select using (auth.uid() = user_id);
create policy "agents_owner_insert" on public.agents for insert with check (auth.uid() = user_id);
create policy "agents_owner_update" on public.agents for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "agents_owner_delete" on public.agents for delete using (auth.uid() = user_id);
